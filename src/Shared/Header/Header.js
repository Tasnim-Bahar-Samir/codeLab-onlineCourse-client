...
const Header = () => {
  ...
  return (
    <div
      className={`flex items-center justify-between bg-gray-200 px-2 md:px-8 shadow-xl mb-2 md:py-5 py-2 ${
        darkMode && "bg-slate-600 text-white"
      }`}
    >
      <div className="">
        <Link
      ...
      <div onBlur={()=>setOpen(false)} className="flex gap-4 lg:gap-6 pr-5 text-md">
        <div 
          className={` z-10 w-full bg-gray-200 ${
            darkMode && "bg-slate-600"
          } flex flex-col items-center md:flex-row gap-4 lg:gap-6 absolute md:static transition-all ease-in-out left-0 ${
        ...
        {user?.uid ? (
          <>
            ...
            <button
              onClick={userSignOut}
              className="px-2 md:px-3 rounded-md border-2 border-orange-600"
            >
              Logout
            </button>
          </>
          ...
          <Link
            className="px-2 md:px-4 py-1 rounded text-white bg-orange-600"
            to="/login"
          >
            Login
          </Link>
